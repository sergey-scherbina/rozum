...
Reads every non-archived scripts/bench/results/*/per-run.csv, aggregates the
...
def main() -> int:
    ...
    doc = {
        ...
        "source": "scripts/bench/export_model_ratings.py (claude driver, rc!=2, non-archived results)",
        "models": models,
    }
    ...
    if dry:
        ...
        OUT.write_text(body)
        print(f"wrote {OUT} ({len(models)} rated models from {len(csvs)} result dirs)")
    return 0
