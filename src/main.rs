...
/// Write (unconditionally overwrite) `~/.claude/commands/rozum-model.md` — a custom Claude Code
/// slash command, `/rozum-model <name>`, that switches the RESIDENT gateway model without
/// restarting the agent session. Possible at all because the gateway ignores a request's `"model"`
/// field for routing (BUG-062-adjacent finding) — it always answers with whatever's resident — so
/// "switching models mid-chat" is just running `rozum gateway switch --model <spec>`, something the
/// agent's own Bash tool can already do; this only gives it a named, discoverable command for that,
/// with the catalog embedded fresh so it's never stale relative to what's actually downloaded.
///
/// Regenerated on every `rozum launch claude` (not written once and left to rot): the list of
/// installed models is baked into the file's TEXT, so `rozum models rm` / a fresh download changes
/// what the NEXT launch writes, with no separate "refresh" step for the operator to remember.
/// User-global (`~/.claude/commands`, not a project's `.claude/commands`) because the resident
/// model is a machine/gateway-wide concept, not tied to any one project's checkout.
fn write_rozum_model_command() {
    let Some(home) = rozum_paths::home_dir() else {
        return; // best-effort: no home dir found → nothing to write into, silently skip
    };
    ...
    let installed = rozum::models::scan_all_installed();
    let catalog = if installed.is_empty() {
        "(none downloaded — run `rozum models list --remote` to see what can be fetched)".to_string()
    } else {
        installed
            .iter()
            .map(|m| format!("- `{}` ({})", m.spec, rozum::models::format_size(m.size_bytes)))
            .collect::<Vec<_>>()
            .join("\n")
    };
    let body = format!(
        r#"---
description: "Switch the resident rozum model without restarting this session"
argument-hint: "<name, e.g. 4b | 9b | 27b | a full spec>"
---

Auto-generated by `rozum launch` — regenerated on every launch, so this list always matches what
is ACTUALLY downloaded right now (never hand-edit; a manual edit is overwritten on the next launch).

The user typed `/rozum-model $ARGUMENTS`. Match `$ARGUMENTS` against the specs below — by size
shorthand (`4b`, `9b`, `27b`), by name substring, or the exact spec — using your own judgment for
a fuzzy/partial match. Currently installed:

{catalog}

If exactly one spec matches, run this via Bash and report what happened (it switches the model
that answers THIS chat too, going forward — no restart of this session):

    rozum gateway switch --model <the matched spec>

If nothing matches (or more than one is plausible and it's not obvious which), do NOT guess —
show the list above and ask which one, or suggest `rozum models list --remote` if they want
something not yet downloaded.
"#
    );
    let _ = std::fs::write(dir.join("rozum-model.md"), body);
}
...
async fn exec_agent(
    ...
    // `/rozum-model` (rozum-model-slash-command): regenerated on every `claude` launch so it
    // always lists whatever's ACTUALLY downloaded right now — add or remove a model with `rozum
    // models rm`/a fresh download, and the very next `rozum launch claude` picks it up with no
    // separate step. Claude-only: custom slash commands are a Claude Code feature. Here, not at
    // one of the three call sites: `exec_agent` is the one place EVERY launch path (plain,
    // verify-gate repair rounds, chain) funnels through before it execs the agent.
    if program.first().map(String::as_str) == Some("claude") {
        write_rozum_model_command();
    }
