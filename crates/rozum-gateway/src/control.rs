...
fn parse_flat_body(body: &str) -> std::collections::HashMap<String, String> {
    let mut out = std::collections::HashMap::new();
    let body = body.trim();
    if body.starts_with('{') {
...
fn messenger_json(args: &[&str]) -> axum::response::Response {
    use axum::response::IntoResponse;
...
async fn messenger_status_route() -> axum::response::Response {
    messenger_json(&["messenger", "status", "--json"])
}

async fn messenger_group_add_route(body: String) -> axum::response::Response {
...
async fn messenger_group_remove_route(body: String) -> axum::response::Response {
    let f = parse_flat_body(&body);
    let registry = f.get("registry").cloned().unwrap_or_else(|| "telegram".into());
    let Some(chat_id) = f.get("chat_id").map(|s| s.trim().to_string()) else {
        return json_err(axum::http::StatusCode::BAD_REQUEST, "chat_id required");
...
async fn messenger_acl_grant_route(body: String) -> axum::response::Response {
    let f = parse_flat_body(&body);
...
async fn messenger_acl_revoke_route(body: String) -> axum::response::Response {
    let f = parse_flat_body(&body);
    let (Some(room), Some(user_id)) = (f.get("room"), f.get("user_id")) else {
        return json_err(axum::http::StatusCode::BAD_REQUEST, "room and user_id required");
    };
...
async fn messenger_bot_service_route(body: String) -> axum::response::Response {
    let f = parse_flat_body(&body);
...
async fn messenger_bot_add_route(body: String) -> axum::response::Response {
    let f = parse_flat_body(&body);
...
async fn messenger_bot_remove_route(body: String) -> axum::response::Response {
    let f = parse_flat_body(&body);
