...
mod tests {
    ...
    // NOTE: the real-MLX router/RAG evals (`model_router_eval`, `rag_worker_eval`)
    // moved to the binary's integration tests (`tests/mlx_evals.rs`) in the workspace
    // split — rozum-agent has no dependency on the mlx engine.
}
