#!/usr/bin/env bash
# Build the UCC's public read routes (slice 1 of `docs/specs/ucc-ssc-backend.md`) to a stable binary.
# Requires the scalascript toolchain (bin/ssc) staged via `./install.sh --dev`.
#
# THIS FILE EXISTS BECAUSE OF BUG-030. The meeting PWA was deployed for six weeks as a binary nobody
# could rebuild, and when its source finally stopped compiling nobody noticed for a week — the only
# artifact of it was the copy already running. A service whose rebuild path is "whatever the author
# typed that day" is one bad restart from being gone. The diagnosis below is that bug's other half:
# a failure here says WHICH kind it is instead of leaving 1,500 lines of cargo output to be read as
# a defect in this repo, which it twice was not.
set -euo pipefail
SSC_ROOT="${SSC_ROOT:-/Users/sergiy/work/my/scalascript}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT="${1:-$HOME/.local/bin/rozum-ucc-ssc}"
...
diagnose_failure() {
  ...
  echo "    SSC_ROOT=/tmp/tc $HERE/build-public-matrix.sh $OUT" >&2
}
...
set +e
"$SSC_BIN" build-rust "$HERE/public-matrix.ssc" -o "$OUT"
rc=$?
set -e
if [ "$rc" -ne 0 ]; then
  diagnose_failure
