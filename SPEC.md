...
- Default model resolution chain for `rozum gateway` / `rozum launch` (highest first): in-process GGUF (`--features gguf`), **in-process native MLX** (`--features mlx-native`, on by default — the primary in-process MLX backend, covering the Qwen3 / Qwen3.6 family with a full native MLX forward, no Python), in-process MLX via `mistralrs` (`--features mistralrs`, opt-in broader-catalog candle fallback), LM Studio HTTP, `ROZUM_BACKEND_URL` env. The Python `mlx_lm.server` HTTP fallback was retired (superseded by the native runtime). If none reachable, both subcommands exit with code 1 rather than serving a placeholder.
