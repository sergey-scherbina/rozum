...
- `nadia` (`crates/nadia`) is the coding agent this project ships itself, and the one client of the gateway that is not third-party: six tools, a jailed workspace, an approval gate, and subagents drivable over HTTP (`nadia serve`) or from the Telegram bot. It is an **app leaf** — it emits neutral OpenAI-form tool JSON and never renders or parses a per-family tool dialect, which stays in the gateway (Contract 1 of `docs/specs/integration.md`). Its contract is `nadia:SPEC.md` in the separate repo, which two further implementations are reviewed against; there is deliberately no second spec here. Reference: `docs/nadia.md`.
