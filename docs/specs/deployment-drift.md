...
Two things this had to survive, both found by its own test on the first run:

- **`#[used]` is not enough.** The compiler keeps the static; the linker dropped it anyway, and the
  scan found nothing in the very binary that declared it. It is an exported (`no_mangle`) static now.
